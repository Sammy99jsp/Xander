...
class XanderAction(abc.ABC):
    ...
    @staticmethod
    def decode(r_action: torch.Tensor) -> "XanderAction":
        """
        Args:
        ...
        match torch.argmax(action[:3]):
            case ActionType.MOVE.value:
                return MoveAction(torch.argmax(action[3:]).item()) # type: ignore
            case ActionType.ATTACK.value:
                return AttackAction(torch.argmax(action[3:]).item()) # type: ignore
...
class Duel(gym.Env[np.ndarray, np.ndarray]):
    def __init__(self, cfg: DuelConfig):
        ...
        w, h = self.combat.arena.grid_dimensions
